...
(function($) {
   ...
   */
   ...
   function smoothScroll(startY, stopY) {
     var rootItem = document.getElementById('root-item');
     var pageMenu = document.getElementById('page_menu');
      if($(window).width() < 640){
        stopY += rootItem.offsetHeight + ($(window).width() / 2) + 40;
        console.log(stopY);
      }
      if($(window).width() >= 640 && $(window).width() < 960 ){
        stopY += rootItem.offsetHeight +40;
        console.log(stopY);
      }
 ...
 i.prototype.tok=function(){
   switch(this.token.type){
     ...
     case"table":{var e="",n,s,i,l,o;e+="<thead>\n<tr>\n";for(s=0;s<this.token.header.length;s++){n=this.inline.output(this.token.header[s]);e+=this.token.align[s]?'<th align="'+this.token.align[s]+'">'+n+"</th>\n":"<th>"+n+"</th>\n"}e+="</tr>\n</thead>\n";e+="<tbody>\n";for(s=0;s<this.token.cells.length;s++){i=this.token.cells[s];e+="<tr>\n";for(o=0;o<i.length;o++){l=this.inline.output(i[o]);e+=this.token.align[o]?'<td align="'+this.token.align[o]+'">'+l+"</td>\n":"<td>"+l+"</td>\n"}e+="</tr>\n"}e+="</tbody>\n";
      return"<div class='table-float'><table>\n"+e+"</table></div>\n"}
